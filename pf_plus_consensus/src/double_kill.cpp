...
pfield consensus() {
    pfield temp;
    double ux = gain_x * (0.5 * ((pf_p1.x + 1.0) - (pf_p2.x + 0.0)) + 0.5 * ((pf_p3.x + 0.0) - (pf_p2.x + 0.0))) - gain_vx * (pf_p1.vx);
    double uy = gain_y * (0.5 * ((pf_p1.y + 0.5) - (pf_p2.y - 1.0)) + 0.5 * ((pf_p3.y + 1.0) - (pf_p2.y - 1.0))) - gain_vy * (pf_p1.vy);
...
void robot_Callback(const nav_msgs::Odometry::ConstPtr& msg) 
{
    ...
    pfield temp;
    //temp = addPfields(attForce(goal, pf_p2), repForce(pf_p1, pf_p2));
    //temp = addPfields(temp, repForce(pf_p3, pf_p2));
    //temp = addPfields(temp, repForce(pf_o1, pf_p2));
    //temp = addPfields(temp, repForce(pf_o2, pf_p2));
    //temp = addPfields(temp, repForce(pf_o3, pf_p2));
