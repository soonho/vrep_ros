...
void gpsCb(const asctec_hl_comm::GpsCustom & msg) {
  ...
  ROS_INFO("Current measurement: %f, %f", msg.latitude, msg.longitude);
  ...
  if (count == g_its) {
    ...
    ros::NodeHandle nh;
    nh.setParam("/fixed/gps_ref_latitude", g_lat_ref);
    nh.setParam("/fixed/gps_ref_longitude", g_lon_ref);
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber gps_sub = nh.subscribe("/fcu/gps_custom", 1, &gpsCb);
