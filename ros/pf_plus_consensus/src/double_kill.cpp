...
PotentialField consensus() {
    PotentialField temp;
    double ux = gain_x * (0.5 * ((pf_p1.x + 1.0) - (pf_p2.x + 0.0)) + 0.5 * ((pf_p3.x + 0.0) - (pf_p2.x + 0.0)) + 0.5 * ((pf_q1.x + 0.5) - (pf_p2.x + 0.0))) - gain_vx * (pf_p2.vx);
    double uy = gain_y * (0.5 * ((pf_p1.y + 0.5) - (pf_p2.y - 1.0)) + 0.5 * ((pf_p3.y + 1.0) - (pf_p2.y - 1.0)) + 0.5 * ((pf_q1.y + 0.5) - (pf_p2.y + 1.0))) - gain_vy * (pf_p2.vy);
    double uz = gain_z * (target_z - pf_p2.z) - gain_vz * (pf_p1.vz);
...
//    att.saturate(max_accxy/2);
    ...
    //para forest
    if (method == 1) {
        rep.add(pf_r1.repForce(pf_o1, pf_p2));
        rep.add(pf_r1.repForce(pf_o2, pf_p2));
        rep.add(pf_r1.repForce(pf_o3, pf_p2));
        rep.add(pf_r1.repForce(pf_o4, pf_p2));
        rep.add(pf_r1.repForce(pf_o5, pf_p2));
    }
    if (method == 2) {
        rep.add(pf_p2.boxForce(pf_p2, 9.25, 2.02, 6.15, -2.17, 10));
        rep.add(pf_p2.boxForce(pf_p2, 9.12, 7.72, 6.17, 3.35, 10));
...
int main(int argc, char **argv) {
    ...
    pub_p1 = n.advertise<geometry_msgs::Quaternion>("/speed_02", 10);
